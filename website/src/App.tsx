...
const APP_STORE_URL = "https://apps.apple.com/us/app/pinglet-ai/id6806096564";
const ANDROID_RELEASED = import.meta.env.VITE_ANDROID_RELEASED === "true";
const PRIMARY_CTA = "Get PingLet for iPhone";
const PUBLIC_SITE_URL = (import.meta.env.VITE_PUBLIC_SITE_URL as string | undefined) || "https://pinglet.ai";
const DOWNLOAD_URL = new URL("/get.html", PUBLIC_SITE_URL).href;
...
const faqs = [
  ["When can I get PingLet?", "PingLet is available for iPhone. Android will follow. Visit our download page for current platform availability."],
...
function DownloadChoices({ compact = false }: { compact?: boolean }) {
  return <div className={`grid items-center gap-8 ${compact ? "lg:grid-cols-[auto_1fr]" : "md:grid-cols-[auto_1fr]"}`}>
    ...
    <div className={compact ? "text-center lg:text-left" : "text-center md:text-left"}>
      <p className="eyebrow text-sage">{"Scan to get PingLet"}</p>
      <h3 className="mt-3 text-2xl font-semibold tracking-[-.035em] sm:text-3xl">{"Your iPhone. A better way to remember."}</h3>
      <p className="mt-3 max-w-md leading-relaxed text-ink/58">{"Get PingLet for iPhone and bring your saved ideas to your Home Screen. Android is coming later."}</p>
...
function DownloadPage() {
  useEffect(() => {
    ...
    else if (/iphone|ipad|ipod/.test(agent) || (/macintosh/.test(agent) && navigator.maxTouchPoints > 1)) {
  ...
  return <main className="page-atmosphere min-h-screen px-5 py-8 sm:px-8">
    ...
    <div className="mx-auto grid min-h-[calc(100vh-7rem)] max-w-5xl place-items-center py-14">
      <section className="w-full rounded-[2.5rem] border border-ink/10 bg-paper/85 p-6 shadow-soft backdrop-blur sm:p-10 lg:p-14">
        <div className="mx-auto mb-10 max-w-2xl text-center"><p className="eyebrow text-clay">Get PingLet</p><h1 className="balance mt-4 text-4xl font-semibold tracking-[-.05em] sm:text-6xl">Take what matters with you.</h1><p className="mt-5 text-lg leading-relaxed text-ink/60">{"Available for iPhone. Keep what you discover, and meet it again on your Home Screen."}</p></div>
        <DownloadChoices />
        <p className="mt-9 text-center text-xs text-ink/42">{ANDROID_RELEASED ? "Scan with your phone to open PingLet in the App Store or Google Play." : "Scan with your iPhone to open PingLet in the App Store. Android is coming soon."}</p>
      </section>
    </div>
  </main>;
}
    ...
    <main>
      <section className="page-atmosphere relative min-h-screen pt-32 sm:pt-40">
        <div className="mx-auto grid max-w-7xl items-center gap-16 px-5 pb-24 sm:px-8 lg:grid-cols-[1.08fr_.92fr] lg:pb-32">
          <div>
            <div className="reveal mb-7 inline-flex items-center gap-2 rounded-full border border-ink/10 bg-white/55 px-4 py-2 text-xs font-semibold"><Icon name="spark" className="h-4 w-4 text-clay" />{"AI memory for your iPhone"}</div>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 text-xs font-semibold text-ink/50"><span>{"FOR IPHONE"}</span><span className="h-1 w-1 rounded-full bg-gold" /><span>PERSONAL SAVES LEAD</span><span className="h-1 w-1 rounded-full bg-gold" /><span>SOURCE PRESERVED</span></div>
          </div>
